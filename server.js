...
app.post("/api/register", async (req, res) => {
    try {
        ...
        const token = jwt.sign({ id: result.rows[0].id, username }, process.env.JWT_SECRET, { expiresIn: "7d" });
...
app.put('/api/projects/:id', auth, async (req, res) => {
    ...
    try {
        const result = await pool.query(
            `UPDATE projects SET name = $1, contract_amount = $2, advance_percent = $3, payment_delay = $4 
...
app.get("/api/transactions/:projectId", auth, async (req, res) => {
    try {
        ...
        const result = await pool.query(
            `SELECT t.* FROM transactions t JOIN projects p ON p.id = t.project_id 
             WHERE t.project_id = $1 AND p.user_id = $2 ORDER BY t.month`,
    ...
    try {
        ...
        if (existing.rows.length > 0) {
            result = await pool.query(
                `UPDATE transactions SET income_accrued = $1, expense = $2, cash_incoming = $3, cash_outgoing = $4 
                 WHERE project_id = $5 AND month = $6 RETURNING *`,
